 ...
 * Created by VISISHTA.
 */
public class TestNGCustomReportListenerSingleExecution implements IReporter{
	...
	private void resultSummary(ISuite suite, IResultMap tests, String testname,
		...
		if (tests.getAllResults().size() > 0) {
			...
			int mq = 0;
			int cq = 0;
			for (ITestNGMethod method : getMethodSet(tests, suite, "result")) {
				...
				if (!className.equalsIgnoreCase(lastClassName)) {
					if (mq > 0) {
						...
						writer.println(">" + lastClassName + "</td>" + buff);//lastClassName
					}
					mq = 0;
					buff.setLength(0);
	 ...
	 */
	private Collection<ITestNGMethod> getMethodSet(IResultMap tests, ISuite suite, String method) {
	...
	public void generateSuiteSummaryReport(List<ISuite> suites) {
		...
		for (ISuite suite : suites) {
			...
			for (ISuiteResult r : tests.values()) {
				...
				int q = getMethodSet(overview.getPassedTests(), suite, "suite").size();
				qty_pass_m += q;
				summaryCell(q, Integer.MAX_VALUE);
				q = getMethodSet(overview.getSkippedTests(), suite, "suite").size();
				qty_skip += q;
				summaryCell(q, 0);
				q = getMethodSet(overview.getFailedTests(), suite, "suite").size();
				qty_fail += q;
				summaryCell(q, 0);
